...
SMTPClient.prototype._onData = function(chunk){
    ...
    if (this._currentAction == this._actionEHLO) {
       var found = str.match(/(smtputf8)/i);
       if (found) {
         this.SMTPUTF8 = true;
       }
    }
...
SMTPClient.prototype.useEnvelope = function(envelope){
    ...
    var isContainUTF8 = (this._envelope.to || []).some(function(item) {
        return !validator.isAscii(item);
    }) || !validator.isAscii(this._envelope.from);

    this.sendCommand("MAIL FROM:<"+(this._envelope.from)+">" + (isContainUTF8 ? " SMTPUTF8" : ""));
};
...
SMTPClient.prototype._actionRCPT = function(str){
    if(Number(str.charAt(0)) != "2"){
        ...
        this._envelope.rcptFailed.push({
            recipient: this._envelope.curRecipient,
            response: str
        });
    }
    ...
    if(!this._envelope.rcptQueue.length){
        if(this._envelope.rcptFailed.length < this._envelope.to.length){
            ...
            this._onError(new Error("Can't send mail - all recipients were rejected"), "RecipientError", this._envelope.rcptFailed);
            return;
        }
    }else{
        this._envelope.curRecipient = this._envelope.rcptQueue.shift();
